...
static void R_AddWorldSurface(msurface_t* surf, shader_t* shader, int dlightMap, int decalBits) {
	...
	R_AddDrawSurf(surf->data, shader, surf->fogIndex, frontFace);
}
...
void R_AddBrushModelSurfaces(trRefEntity_t* ent) {
	...
	for (i = 0; i < tr.refdef.numDecalProjectors; i++)
	{
		...
		VectorAdd(bmodel->bounds[0], tr.or.origin, mins);
		VectorAdd(bmodel->bounds[1], tr.or.origin, maxs);

		// set bit
		if (R_TestDecalBoundingBox(&tr.refdef.decalProjectors[i], mins, maxs)) {
			R_TransformDecalProjector(&tr.refdef.decalProjectors[i], tr.or.axis, tr.or.origin, &localProjectors[numLocalProjectors]);
			numLocalProjectors++;
			decalBits <<= 1;
			decalBits |= 1;
		}
...
void R_AddWorldSurfaces(void) {
	...
	//else
	{
		...
		R_RecursiveWorldNode(tr.world->nodes, 255, 0, tr.refdef.decalBits);
